...
Adafruit_TFTDMA::Adafruit_TFTDMA(int8_t tc, int8_t reset, int8_t cs,
                                 int8_t cd, int8_t rd, int8_t wr, int8_t d0, _EPioType periph) : Adafruit_GFX(TFTWIDTH, TFTHEIGHT)
{
    ...
    if ((tc < 0) || (tc >= (int8_t)NUM_TIMERS))
        return;                                 // Invalid TC number!
    uint8_t dBit = g_APinDescription[d0].ulPin; // d0 bit # in PORT
#if TFT_INTERFACE == TFT_INTERFACE_8
    if (dBit & 7)
    {
        //Serial.println(dBit);
        //Serial.println("Not Byte aligned Interface 8");
        ...
        return; // d0 is not byte-aligned in PORT!
    }
#elif TFT_INTERFACE == TFT_INTERFACE_16
    if (dBit & 15)
        return; // d0 is not halfword-aligned in PORT!
#endif
    ...
    if (cs >= 0)
    { // If chip select pin is specified...
    ...
    { // No CS specified (tied low)
        csPinMask = 0;
        csPortSet = csPortClr = NULL;
    }
    if (rdPin >= 0)
    { // If read-strobe pin is specified...
    ...
    wrPeripheral = periph;
    if (periph == PIO_CCL)
    { // WR pin is using CCL to invert PWM (but not GPIO)
    ...
    { // WR pin is regular PWM out; requires external inverter
...
bool Adafruit_TFTDMA::begin(void)
{
    ...
    digitalWrite(cdPin, HIGH); // All idle
    ...
    writeReg8(ILI9341_MADCTL, ROTATION0);
...
TFT_framebuffer::TFT_framebuffer(int8_t tc, int8_t reset, int8_t cs,
                                 int8_t cd, int8_t rd, int8_t wr, int8_t d0, _EPioType periph) : Adafruit_TFTDMA(tc, reset, cs, cd, rd, wr, d0, periph)
{
}

bool TFT_framebuffer::begin(void)
{
    if (Adafruit_TFTDMA::begin())
        return true;
...
void TFT_framebuffer::setRotation(byte rotation)
{
    CS_ACTIVE;
    writeReg8(ILI9341_SOFTRESET, 0);
    delay(150);
    ...
    writeReg16(ILI9341_VCOMCONTROL1, 0x2B2B);
    writeReg8(ILI9341_VCOMCONTROL2, 0xC0);
    writeReg8(ILI9341_MADCTL, rotation);
    writeReg8(ILI9341_PIXELFORMAT, 0x55);
    writeReg16(ILI9341_FRAMECONTROL, 0x001B);
    writeReg8(ILI9341_ENTRYMODE, 0x07);
    /* writeReg32(ILI9341_DISPLAYFUNC, 0x0A822700);*/
    ...
    int16_t x1, int16_t y1, int16_t w, int16_t h, uint16_t color)
{
    ...
    // Off-screen clipping
    if ((x1 >= TFTWIDTH) || (y1 >= TFTHEIGHT) || (x2 < 0) || (y2 < 0))
        return;
...
TFT_segmented::TFT_segmented(int8_t tc, int8_t reset, int8_t cs, int8_t cd,
                             int8_t rd, int8_t wr, int8_t d0, _EPioType periph) : Adafruit_TFTDMA(tc, reset, cs, cd, rd, wr, d0, periph)
{
}
                           ...
                           void *userData)
{
    ...
    if (maxSegmentLines > height)
        maxSegmentLines = height;
