...
class Serial_Commands
{
  ...
  void init(void);
  uint16_t GetDiameter(void);
  uint8_t GetDiameterDecimalLocation(void);
  uint16_t GetDiameterError (void);
  char *GetSpoolRPM(void);
  char *GetPullerRPM(void);
  bool DiameterHasNewData(void);
  bool HasFilamentDiameterError(void);
  bool SpoolHasNewData(void);
  bool PullerHasNewData(void);
  protected:
  private:
